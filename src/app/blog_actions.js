...
export async function FetchBlogs(search, category, sort, user) {
    try {
        const access_token = cookies().get('_access')?.value
        const res = await fetch(`${process.env.API_URL}/api/v1/blogs/?user=${user?user:""}&sort=${sort?sort:""}&category=${category?category:""}&search=${search?search:""}`, {
            method: 'GET',
            headers: {
